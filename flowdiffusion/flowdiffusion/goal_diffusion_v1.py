...
class GoalGaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, vid_cond, task_embed,  clip_x_start=False, rederive_pred_noise=False):
        ...
        x_1 = torch.cat([x_1, vid_cond], dim=1) ##[32,6,6,48,64]
    ...
    def p_mean_variance(self, x, t, vid_cond, task_embed,  clip_denoised = False):
        preds = self.model_predictions(x, t, vid_cond, task_embed)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, vid_cond, task_embed):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x, batched_times, vid_cond, task_embed, clip_denoised = True)
        noise = torch.randn_like(x) if t > 0 else 0. # no noise if t == 0
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start

    @torch.no_grad()
    def p_sample_loop(self, shape, vid_cond, task_embed, return_all_timesteps=False):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            ...
            img, x_start = self.p_sample(img, t, vid_cond, task_embed)
            imgs.append(img)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, vid_cond, task_embed, return_all_timesteps=False):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            # self_cond = x_start if self.self_condition else None
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, vid_cond, task_embed, clip_x_start = False, rederive_pred_noise = True)
    ...
    @torch.no_grad()
    def sample(self, vid_cond, task_embed, batch_size = 16, return_all_timesteps = False):
        image_size, channels = self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, image_size[0], image_size[1]), vid_cond, task_embed,  return_all_timesteps = return_all_timesteps)
...
class Trainer(object):
    ...
    def sample(self, vid_conds, tasks):
        assert vid_conds.shape[0] == len(tasks)
        
        device = self.device
        bs = vid_conds.shape[0]
        x_2s = vid_conds.to(device)
        tasks = self.encode_batch_text(tasks).to(device)

        with self.accelerator.autocast():
            output = self.ema.ema_model.sample(batch_size=bs, x_cond=x_2s, task_embed=tasks)
        return output    
